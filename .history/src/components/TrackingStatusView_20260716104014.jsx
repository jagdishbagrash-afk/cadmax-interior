...
export default function TrackingStatusView({
  ...
  }, [carrier, labelData, order?.orderId, order?.orderNumber, payload?.orderNumber, shipment?.orderNumber, trackingNumber]);
  ...
  const refreshShipmentDetails = async () => {
    ...
    return main.GetOrderShipment(orderId);
  };
                  ...
                  >
                    <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <p className="text-xs text-gray-500">
                        {stepTime
                          ? moment(stepTime).isValid()
                            ? moment(stepTime).format("DD MMM YYYY, hh:mm A")
                            : stepTime
                          : "Time unavailable"}
                      </p>
                    </div>
