...
export default function CommercialDesign() {
    const commercialServices = [
        { title: "3D MODELING & BRAND-CENTRIC VISUALIZATION", span: "row-span-2" },
        { title: "2D SPACE PLANNING & OPTIMIZATION" },
        { title: "CORPORATE OFFICES & WORKSPACES" },
        { title: "CAFÉS, SALONS & SERVICE STUDIOS" },
        { title: "EXTERIOR & FACADE DESIGN" },
        { title: "RETAIL STORES & SHOWROOMS" },
        { title: "LIGHTING, SIGNAGE & MATERIAL COORDINATION" },
    ];
    return (
        <div
 ...
 tracking-[-0.02em] text-center uppercase Creato  ">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 auto-rows-[300px] gap-6">
                    {commercialServices?.map((item, index) => (
                        ...
                        >
                            <Image
                                src={imagesrc?.src} // replace with actual image
                                alt={item.title}
                                fill
